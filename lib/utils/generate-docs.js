...
async function createResourceIndexContent(serviceName, resourceName, resourceType, resourceIdentifiers, schemaName, schema, componentsSchemas, resourceData) {
    ...
    let resourceDescription = `Creates, updates, deletes or gets a <code>${pluralize.singular(resourceName)}</code> or lists <code>${resourceName}</code> in a region`;
    ...
    if (resourceType === 'native') {
        isPlural = false;
        resourceData['x-example-where-clause'] ? sqlExampleWhere = `${resourceData['x-example-where-clause']};` : noSelect = true;
        if(!noSelect){
...
## Fields
${!noSelect ? `<table><tbody><tr><th>Name</th><th>Datatype</th><th>Description</th></tr>${generateFieldsTable(schema.properties, componentsSchemas, schemaName, false, resourceType === 'native')}</tbody></table>` : '<code>SELECT</code> operation not supported for this resource.'}

## Methods
${generateMethodsTable(serviceName, resourceName, sqlVerbsList)}

${!noSelect ? `## ${mdCodeAnchor}SELECT${mdCodeAnchor} examples
List all <code>${pluralize.plural(resourceName)}</code> in a region.
${sqlCodeBlockStart}
${sqlExampleSelect}
${sqlExampleListCols}
${sqlExampleFrom}
${sqlExampleListWhere}
${codeBlockEnd}
Gets all properties from a <code>${resourceName}</code>.
${sqlCodeBlockStart}
${sqlExampleSelect}
${sqlExampleGetCols}
${sqlExampleFrom}
    ...
    if (isPlural) {
        if(Array.isArray(properties)){
            ...
            properties.forEach(propName => {
                ...
                const type = propDetails && propDetails.type ? propDetails.type : 'FRED';
