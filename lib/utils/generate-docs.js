...
// const StackQL = require('@stackql/stackqljs');
...
async function createResourceIndexContent(serviceName, resourceName, resourceType, resourceIdentifiers, schemaName, schema, componentsSchemas, resourceData) {
    ...
    async function getSqlVerbsFromStackql(serviceName, resourceName) {
        const command = `./stackql exec --registry='{"url": "file:///mnt/c/LocalGitRepos/stackql/aws-cfn-schema-to-openapi", "localDocRoot": "/mnt/c/LocalGitRepos/stackql/aws-cfn-schema-to-openapi", "verifyConfig": {"nopVerify": true}}' --output json "show methods in aws.${serviceName}.${resourceName}"`;
...
## Fields
${!noSelect ? `<table><tbody><tr><th>Name</th><th>Datatype</th><th>Description</th></tr>${generateFieldsTable(fields, componentsSchemas, schemaName, isPlural, resourceType === 'custom')}</tbody></table>` : '<code>SELECT</code> operation not supported for this resource.'}

## Methods
${generateMethodsTable(serviceName, resourceName, sqlVerbsList)}

${!noSelect ? `## ${mdCodeAnchor}SELECT${mdCodeAnchor} Example
${sqlCodeBlockStart}
${sqlExampleSelect}
${sqlExampleCols}
${sqlExampleFrom}
