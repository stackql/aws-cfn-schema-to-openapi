...
async function createResourceIndexContent(serviceName, resourceName, resourceType, resourceIdentifiers, schemaName, schema, componentsSchemas, resourceData) {
    ...
    const sqlExampleListCols = getColumns(fields, true, resourceType === 'native');
    const sqlExampleGetCols = getColumns(schema.properties, false, resourceType === 'native');
...
## Fields
${isSelectable || hasList || hasGet ? `<table><tbody><tr><th>Name</th><th>Datatype</th><th>Description</th></tr>${generateFieldsTable(schema.properties, componentsSchemas, schemaName, false, resourceType === 'native')}</tbody></table>` : '<code>SELECT</code> operation not supported for this resource.'}

## Methods
${generateMethodsTable(serviceName, resourceName, sqlVerbsList)}

${generateSelectExamples(resourceName, hasList, hasGet, sqlExampleSelect, sqlExampleListCols, sqlExampleFrom, sqlExampleListWhere, sqlExampleGetCols, sqlExampleGetWhere)}
...
function getColumns(properties, isList, isCustom = false){
    let columns = 'region,';
    if (isList) {
...
function generateFieldsTable(properties, componentsSchemas, schemaName, isList, isCustom = false) {
    let fieldsTable = '';
    ...
    if (isList) {
        if(Array.isArray(properties)){
            ...
            properties.forEach(propName => {
                ...
                const type = propDetails && propDetails.type ? propDetails.type : '';
                fieldsTable += generateTableRow(propName, type, cleanDescription(propDetails && propDetails.description ? propDetails.description : ''), isCustom);
            });
        }
...
function generateSelectExamples(resourceName, hasList, hasGet, sqlExampleSelect, sqlExampleListCols, sqlExampleFrom, sqlExampleListWhere, sqlExampleGetCols, sqlExampleGetWhere){
    let returnString = '';
    if(hasList || hasGet){
    ...
    if(hasList){
        const listDesc = `List all <code>${pluralize.plural(resourceName)}</code> in a region.`;
