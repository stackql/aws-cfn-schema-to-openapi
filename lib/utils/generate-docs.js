...
async function createDocsForService(yamlFilePath) {
    ...
    firstColumn.forEach(async (resource) => {
        ...
        const resourceIndexContent = await createResourceIndexContent(serviceName, resource.name, resource.type, resource.identifiers, resource.schemaName, resource.schema, resource.componentsSchemas, resource.resourceData, resource.schemaTypeName);
...
async function createResourceIndexContent(serviceName, resourceName, resourceType, resourceIdentifiers, schemaName, schema, componentsSchemas, resourceData, schemaTypeName) {
...
## Fields
${isSelectable || hasList || hasGet ? `<table><tbody><tr><th>Name</th><th>Datatype</th><th>Description</th></tr>${generateFieldsTable(serviceName, resourceName, resourceType, schemaName, schema, componentsSchemas, sqlExampleListCols)}</tbody></table>` : '<code>SELECT</code> operation not supported for this resource.'}
