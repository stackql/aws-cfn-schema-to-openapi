...
function createDocsForService(yamlFilePath) {
    ...
    for (let resourceName in resourcesObj) {
        ...
        resources.push({ name: resourceName, type: methodType, identifiers: identifiers, schema, componentsSchemas });
    }
    ...
    firstColumn.forEach((resource) => {
        ...
        const resourceIndexContent = createResourceIndexContent(serviceName, resource.name, resource.type, resource.identifiers, resource.schema, resource.componentsSchemas);
...
function createResourceIndexContent(serviceName, resourceName, resourceType, resourceIdentifiers, schema, componentsSchemas) {
    ...
    let sqlExample = `SELECT *<br/>FROM aws.${serviceName}.${resourceName}<br/>WHERE region = 'us-east-1'`;
    if(resourceType === 'get') {
        resourceIdentifiers.forEach(identifier => {
            sqlExample += `<br/>AND data__Identifier = '&lt;${identifier}&gt;'`;
        });
    };
    return `---
title: ${resourceName}
...
${generateFieldsTable(schema.properties, componentsSchemas)}
</tbody></table>
...
function generateFieldsTable(properties, components) {
    let fieldsTable = '';
    for (let propName in properties) {
        const prop = properties[propName];
        let type = prop.type;
        if (prop.hasOwnProperty('$ref')) {
            // Use the reference to get the actual type from components
            type = getTypeFromRef(prop['$ref'], components);
        }
        const descriptionCleaned = (prop.description || '')
        .replace(/\n/g, '<br/>')
        .replace(/\//g, '&#x2F;')
        ...
        .replace(/\[/g, '&#91;')
        .replace(/\]/g, '&#93;');
        fieldsTable += `<tr><td><code>${propName}</code></td><td><code>${type}</code></td><td>${descriptionCleaned}</td></tr>\n`;
    }
    return fieldsTable;
}
