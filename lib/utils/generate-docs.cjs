...
function createResourceIndexContent(serviceName, resourceName, resourceType, resourceIdentifiers, schemaName, schema, componentsSchemas) {
    ...
    const sqlExampleFrom = `FROM aws.${serviceName}.${resourceName}`;
    let sqlExampleWhere = `WHERE region = 'us-east-1'`;
    let isPlural = true;
...
function generateTableRow(propName, type, descriptionCleaned) {
    return `<tr><td><code>${toSnakeCase(propName)}</code></td><td><code>${type}</code></td><td>${descriptionCleaned}</td></tr>\n`;
}
