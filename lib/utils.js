...
export function convertToOpenAPI(input, componentName, schemaDefinitions) {
  ...
  for (const [key, value] of Object.entries(input.properties)) {
    ...
    const needToRemoveObjectType = hasObjectInType && !value.$ref && Array.isArray(value.type);
...
export function generateStackqlViews(openApiSpec) {
    ...
    const views = topLevelSchemas.map(([schemaName, schema]) => {
      ...
      const formattedQuery = removeExtraEmptyLines(selectQuery);
      return {
        [resource]: {
          name: resource,
  ...
  export function removeEmptyLinesInYaml(yamlString) {
    return yamlString.replace(/^\s*[\r\n]/gm, '');
  }